...
venues:
  ...
  okx:
    ...
    context.engine_state.order_books = {
        "binance": {"BTC/USDT": {"bid": 100.0, "ask": 101.0, "timestamp": now}},
        "okx": {"BTC/USDT": {"bid": 103.0, "ask": 104.0, "timestamp": now}},
    }
    app = create_app(context)
    async with AsyncClient(app=app, base_url="http://testserver") as client:
...
@pytest.mark.asyncio
async def test_validate_and_apply_runtime(tmp_path: Path) -> None:
    config_path = tmp_path / "config.yaml"
    config_path.write_text(
        """
mode: paper
...
venues:
  ...
  okx:
    ...
    app = create_app(context)
    async with AsyncClient(app=app, base_url="http://testserver") as client:
        validate = await client.post("/api/ui/config/validate", json={"profile": "paper"})
        assert validate.status_code == 200
        applied = await client.post(
            "/api/ui/config/apply",
            json={"min_spread_bps": 25.0, "default_notional_usd": 75.0},
        )
        assert applied.status_code == 200
        runtime = applied.json()["engine"]
        assert runtime["min_spread_bps"] == 25.0
        assert context.engine.runtime_config()["min_spread_bps"] == 25.0
